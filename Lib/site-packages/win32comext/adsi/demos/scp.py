...
* Adds and removes an ActiveDirectory "Services Connection Point",
  including managing the security on the object.
* Creates and registers Services Principal Names.
...
would:
* Attempt to delete a Services Connection Point for the service class
  'PythonScpTest'
* Attempt to create a Services Connection Point for that class, with 2
  keywords and a binding string of 'bind_info'
* Create a Services Principal Name for the service and register it
...
* Delete the Services Connection Point
...
def ScpCreate(
    ...
    service_class_name,  # Services class string to store in SCP.
...
# "Enabling Services Account to Access SCP Properties"
# From that article:
...
def AllowAccessToScpProperties(
    accountSAM,  # Services account to allow access.
...
# Services Principal Names functions from the same sample.
...
def do_ScpCreate(po):
    """Create a Services Connection Point"""
    global g_createdSCP
    scp = ScpCreate(
...
def do_ScpDelete(po):
    """Delete a Services Connection Point"""
...
def do_SpnCreate(po):
    """Create a Services Principal Name"""
...
def do_SpnRegister(po):
    """Register a previously created Services Principal Name"""
...
def do_SpnUnregister(po):
    """Unregister a previously created Services Principal Name"""
    if not g_createdSPNLast:
